...
def extract_MFCC(wav_file):
    ...
    # 提取特征
    fea = librosa.feature.mfcc(
        y, sr, n_mfcc=12, n_mels=24, n_fft=256, win_length=256, hop_length=80, lifter=12)
    # 进行正则化
...
if __name__ == "__main__":
    ...
    for i in range(1, 15):
        ...
        N_state = 4
        N_mix = 3
        pi, A, hmm_means, hmm_sigmas, hmm_ws = init_para_hmm(
            collect_fea, N_state, N_mix)
        ...
        train_GMMHMM.fit(np.concatenate(
            collect_fea, axis=0), np.array(len_feas))
    ...
    for i in range(98):
        ...
        print("true lab  %d det lab1 %d  det lab2 %d " %
              (lab_true, det_lab, det_lab2))
